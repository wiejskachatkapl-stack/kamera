...
try {$listener.Start()} catch {
  Write-Host "Nie moge uruchomic portu 8090. Szczegoly: $($_.Exception.Message)" -ForegroundColor Red
  throw
}
Write-Host "Kamera LIVE v1010 - serwer PowerShell" -ForegroundColor Cyan
Write-Host "PC: http://127.0.0.1:8090" -ForegroundColor Green
Write-Host "Telefon Wi-Fi: http://192.168.0.2:8090" -ForegroundColor Green
...
while($listener.IsListening){
  ...
  try{
    ...
    if($path -eq "/stream.ts"){
      ...
      }catch{} finally {$input.Dispose();$up.Dispose()}
    } else {
